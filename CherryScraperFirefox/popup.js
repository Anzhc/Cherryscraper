...
function setDefaultOptions() {
    browser.storage.local.get(['buttonEnabled', 'selectedSite', 'siteList', 'downloadDirectory'], function(data) {
        if (data.buttonEnabled === undefined) {
            browser.storage.local.set({buttonEnabled: true});  // Set default value for buttonEnabled
        }
        if (data.selectedSite === undefined) {
            browser.storage.local.set({selectedSite: 'danbooru.donmai.us'});  // Set default value for selectedSite
        }
        if (data.siteList === undefined) {
            browser.storage.local.set({siteList: ['danbooru.donmai.us', 'safebooru.org', 'tbib.org', 'derpibooru.org', 'rule34.xxx','sankakucomplex.com','gelbooru.com','yande.re','e621.net', 'realbooru.com','chounyuu.com', 'yandex.ru', 'nozomi.la', 'othersite.com']});  // Set default value for siteList
        }
        if (data.downloadDirectory === undefined) {
            browser.storage.local.set({downloadDirectory: 'dataset'});  // Set default value for downloadDirectory
        }
    });
}
...
browser.storage.onChanged.addListener(function(changes, areaName) {
    if (areaName === 'local' && (changes.selectedSite || changes.downloadDirectory)) {
        restoreOptions();
    }
});

document.querySelector('#enableButton').addEventListener('input', function() {
    browser.storage.local.set({buttonEnabled: this.checked});
});

document.querySelector('#selectedSite').addEventListener('change', function() {
    browser.storage.local.set({selectedSite: this.value});
});

document.querySelector('#downloadDirectory').addEventListener('input', function() {
    browser.storage.local.set({downloadDirectory: this.value});
});
